...
public class SocketProcessor implements Runnable {
  ...
  private long nextSocketId = 16
      * 1024; //start incoming socket ids from 16K - reserve bottom ids for pre-defined sockets (servers).
    ...
    if (fullMessages.size() > 0) {
      for (Message message : fullMessages) {
        ...
        this.messageProcessor.process(message,
            this.writeProxy);  //the message processor will eventually push outgoing messages into an IMessageWriter for this socket.
      }
      fullMessages.clear();
    }
    ...
    while (outMessage != null) {
      ...
      if (socket != null) {
        ...
        if (messageWriter.isEmpty()) {
          ...
          emptyToNonEmptySockets.add(
              socket);    //not necessary if removed from nonEmptyToEmptySockets in prev. statement.
        } else {
          messageWriter.enqueue(outMessage);
        }
      }
